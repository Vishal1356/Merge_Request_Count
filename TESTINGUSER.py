...
def fetch_merge_requests(Repo_Name,user_id, private_token):
    api_url = f"https://git.automate.com/api/v4/projects/testing/{Repo_Name}/merge_requests?scope=all&state=merged&author_id={user_id}"
    headers = {"Private-Token": private_token}
...
def main():
    ...
    for user_id, username in user_data.items():
        ...
        merge_requests_data = fetch_merge_requests(Repo_Name,user_id, private_token)
        if merge_requests_data:
            # Create DataFrame with merge requests data for this user
            df_user_merge_requests = pd.DataFrame({"Repository Name ": [Repo_Name],
                                                   "Name": [username],
